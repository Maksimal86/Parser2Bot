...
title_of_product = 'Влажный корм для кошек FLORIDA кусочки с тунцом и шпинатом в соусе 12 шт по 85 гр'
def search_of_mass_product(title_of_product):
    try:
        if search_for_mass_composite_product_kg(title_of_product):
            ...
            print('Штучный товар ', title_of_product)
...
def search_for_mass_composite_product_kg(title_of_product):
    match = re.search('\d*\s{,1}шт\s{1,}\d{1,3}\S{,1}\d{,3}\s{0,}кг', title_of_product)
    #print(match)
    if match:
...
if __name__ == '__main__':
    print(search_of_mass_product(title_of_product='24" Монитор HOGA 24-дюймовый изогнутый без полей, 75 Гц, черный, черный'))