...
class Reference_Ozon(Browser):
    ...
    def get_mass_product_in_kg(self, teg):
        massa_of_product_in_kg = testre.search_of_mass_product(Reference_Ozon.get_title(self, teg))
    ...
    def get_dict_results_for_products(self):  # доделать из-за списка списков
        price_list_of_products = Reference_Ozon.get_price_list_of_products(self)
        dict_price_list_of_piece_goods = {}
    ...
    def get_result_for_bot(self):
        n = 0
        result_for_bot = []
        for key in Reference_Ozon.sorting_keys_of_dict(self):
            n += 1
            print('№', n, str(key) + '-' + Reference_Ozon.get_dict_results_for_products(self)[key].translate({ord(i): " " for i in "'' "}))
            result_for_bot.append(('№', n, str(key) + '-' + Reference_Ozon.get_dict_results_for_products(self)[key].translate({ord(i): " " for i in "'' "})))
...
class Reference_Sber(Browser):
    ...
    def get_mass_product_in_kg(self, card):
        return testre.search_of_mass_product(Reference_Sber.get_title(self, card))
...
def main_function_get_product_data(reference):
    ...
    for i in range(5):
        ...
        received_link.get_result_for_bot()
        received_link.get_reference_on_page(reference)
